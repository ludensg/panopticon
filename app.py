...
def create_demo_garden_with_sample_content(name: str = "Demo Garden") -> GardenState:
    """
    Create a demo garden with:
    - One child (like create_default_garden) but named + themed for demo.
    - A pre-populated feed.
    - One normal DM conversation.
    - One simulation session with at least a minimal outcome.
    ...
    default_config = ChildConfig(
        ...
        interests=[
            Interest("space", 0.5),
            Interest("animals", 0.3),
            Interest("drawing", 0.2),
        ],
        mode="realistic",
        max_posts=8,
    )
    child = garden.add_child(default_config)

    # Ensure the child has a profile in garden.profiles
    ...
    try:
        ...
        demo_post = Post(
            ...
            text=(
                "This is a demo post showing how the feed looks. "
                "In a real run, posts are generated by the AI based on topics like space and animals."
            ),
            topic="space",
    ...
    friend_profile = Profile(
        id=make_id("profile"),
        role="friend",
        display_name="Jamie (friend)",
        avatar_style="realistic",
        personality_tags=["supportive", "curious"],
        topics=["school", "games"],
        is_parent_controlled=True,
        avatar_hue_shift=random.random(),
    )
    garden.profiles.append(friend_profile)

    conv_id = f"conv_{child.id}_{friend_profile.id}"
    now = datetime.utcnow()
    ...
    demo_dm_1 = DMMessage(
        ...
        sender_profile_id=friend_profile.id,
        receiver_profile_id=child.profile_id,
        text="Hey, did you see that cool space video they showed in class today?",
        created_at=now,
    )
    demo_dm_2 = DMMessage(
        id=make_id("dm"),
        ...
        receiver_profile_id=friend_profile.id,
        text="Yeah! The rocket launch was awesome. I want to learn more about it.",
        created_at=now,
    )
    child.dm_messages.extend([demo_dm_1, demo_dm_2])
    ...
    if scenarios:
        ...
        try:
            ...
            try:
                ...
                backend_sim, model_sim = ("openai", "gpt-4.1-mini")

            try:
                event, _first_msg = start_simulation_session(
                    garden=garden,
                    child=child,
                    scenario_id=scen.id,
                    partner_profile_id=sim_profile.id,
                    backend=backend_sim,
            ...
            if event is not None:
                ...
                reply = DMMessage(
                    id=make_id("dm"),
                    child_id=child.id,
                    conversation_id=conv_id_sim,
                    sender_profile_id=child.profile_id,
                    receiver_profile_id=sim_profile.id,
                    text=(
                        "I'm not sure I should share that. "
                        "I want to talk to my parent or teacher first."
                    ),
                ...
                # Try to auto-evaluate the simulation
                try:
                    ...
                    if not getattr(event, "evaluation_summary", None):
                        event.evaluation_summary = (
                            "Demo summary: the child hesitated to share private information. "
                            "Use this view to inspect full transcripts and see how simulations work."
                        )
                    event.is_active = False
        except Exception:
...
def init_session_state():
    if "gardens" not in st.session_state:
        ...
        # Use a demo garden with pre-populated content for the evaluator
        demo_garden = create_demo_garden_with_sample_content(name="Demo Garden")
        st.session_state.gardens[demo_garden.id] = demo_garden
        st.session_state.active_garden_id = demo_garden.id

        # Set active_child_id to the only child in this garden
        only_child = next(iter(demo_garden.children.values()))
